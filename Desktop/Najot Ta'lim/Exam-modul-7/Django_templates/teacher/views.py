...
class Get_info_api:
    ...
    @staticmethod
    def post_teacher(first_name, last_name, email, phone_number, username, password):
        url = 'http://127.0.0.1:8000/api/teacher/'
        data = {
...
class Login_View(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (username1 and password):
            print('aaaaaaaaaaaaaaaaaaaaaaaaaaaa', Get_info_api.get_teacher())
            for username_from_api1 in Get_info_api.get_teacher():
            ...
            for username_from_api1 in Get_info_api.get_teacher():
                ...
                if phone_number != username_from_api1['phone_number']:
                    ...
                    if username != username_from_api1['username']:
                        if Get_info_api.post_teacher(first_name=first_name, last_name=last_name, email=email, phone_number=phone_number, username=username, password=password) != 'failed':
                            status = request.session.get('status')
                            request.session['status'] = 'tch'
                            login(request, user)
                            return redirect('login-t')
